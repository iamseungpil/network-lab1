#!/usr/bin/env python3
"""
Fixed SDN Controller for Ring Topology
- Proper STP implementation with timer
- Fixed flooding prevention
- Clear Dijkstra path logs
"""
...
class FixedRingController(app_manager.RyuApp):
    OFP_VERSIONS = [ofproto_v1_3.OFP_VERSION]
    _CONTEXTS = {'switches': switches.Switches}

    def __init__(self, *args, **kwargs):
        super(FixedRingController, self).__init__(*args, **kwargs)
    ...
    def print_header(self):
        ...
        print("\n" + "="*80)
        print("🚀 FIXED SDN CONTROLLER - RING TOPOLOGY")
        print("="*80)
        print(f"⏰ Started at: {datetime.now().strftime('%H:%M:%S')}")
        print("🔍 Features: Auto STP, Flooding Prevention, Dijkstra Routing")
        print("="*80 + "\n")
    
    def _monitor_topology(self):
        """Periodically check topology stability and compute STP"""
        while True:
            hub.sleep(3)  # Check every 3 seconds
    ...
    def switch_features_handler(self, ev):
        ...
        print(f"✅ [OPENFLOW] Switch s{dpid} ready")
    ...
    def link_delete_handler(self, ev):
        ...
        # Mark for STP recomputation
        self.last_topology_change = time.time()
        self.topology_stable = False
        
        print(f"🔄 [RECOVERY] Topology updated, STP will recompute")
        print("="*60 + "\n")
    ...
    def check_topology_stability(self):
        ...
        # Need at least 10 switches for ring topology
        if self.topology_graph.number_of_nodes() < 10:
            return
        
        # Wait 5 seconds after last change for topology to stabilize
        if current_time - self.last_topology_change > 5.0:
    ...
    def compute_spanning_tree(self):
        ...
        try:
            ...
            if not nx.is_connected(self.topology_graph):
                print("⚠️  [STP] Graph not connected, skipping STP")
            ...
            # Also add reverse edges to tree_edges
            tree_edges_bidirectional = tree_edges.copy()
            for u, v in tree_edges:
                tree_edges_bidirectional.add((v, u))
            
            blocked_edges = all_edges - tree_edges_bidirectional
    ...
    def packet_in_handler(self, ev):
        ...
        if (dpid, in_port) in self.blocked_ports:
            # Silently drop packets on blocked ports
            return
        
        # Handle broadcast/multicast
        if self.is_broadcast_multicast(dst_mac):
            # Check if we should flood this broadcast
    ...
    def calculate_shortest_path(self, src_dpid, dst_dpid):
        ...
        try:
            # Use spanning tree for path calculation if available
            if self.spanning_tree and nx.is_connected(self.spanning_tree):
                if self.spanning_tree.has_node(src_dpid) and self.spanning_tree.has_node(dst_dpid):
                    path = nx.shortest_path(self.spanning_tree, src_dpid, dst_dpid)
                    return path
            
            # Fallback to full graph
            if nx.has_path(self.topology_graph, src_dpid, dst_dpid):
    ...
    def flood_packet(self, datapath, msg, in_port):
        ...
        # Only flood if we have ports available
        if flood_ports:
