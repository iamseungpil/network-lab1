...
# Dual Controller Startup Script with Conda Environment

echo "Starting Dual Controller SDN Environment (Conda)..."
...
echo "Activating conda environment: sdn-env"
source /data/miniforge3/etc/profile.d/conda.sh
conda activate sdn-env

# Verify RYU is working
if ! ryu-manager --version > /dev/null 2>&1; then
    echo "ERROR: RYU not working in conda environment!"
    echo "Please run: conda activate sdn-env && ryu-manager --version"
    exit 1
fi

echo "✓ RYU version: $(ryu-manager --version)"
...
if ! netstat -ln | grep ":6633 " > /dev/null; then
    echo "WARNING: Primary controller (6633) may not be ready"
fi

if ! netstat -ln | grep ":6634 " > /dev/null; then
    echo "WARNING: Secondary controller (6634) may not be ready"
fi

# Start Mininet with dual controller topology (window 2)
echo "Starting Mininet with dual controller topology..."
tmux send-keys -t $SESSION_NAME:2 "source /data/miniforge3/etc/profile.d/conda.sh && conda activate sdn-env && sudo python3 mininet/dijkstra_graph_topo.py" Enter
...
echo ""
echo "Environment: conda sdn-env (Python 3.8, RYU 4.34, eventlet 0.30.2)"
echo ""